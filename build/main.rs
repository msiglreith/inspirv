...
fn main() {
    ...
    println!("cargo:rerun-if-changed=grammar/extinst.glsl.std.450.grammar.json");
    ...
    let grammar_core = json::parse(include_str!("../grammar/spirv.core.grammar.json")).unwrap();
    let grammar_glsl = json::parse(include_str!("../grammar/extinst.glsl.std.450.grammar.json")).unwrap();
    ...
    build_core_mod(core_dir.join("mod.rs"), &grammar_core).unwrap();
    build_instructions(core_dir.join("instruction.rs"), &grammar_core).unwrap();
    build_enum(core_dir.join("enumeration.rs"), &grammar_core).unwrap();

    // Generate code for the GLSL instruction set of the SPIR-V specification
    let glsl_dir = Path::new("src/glsl");
    DirBuilder::new().recursive(true).create(glsl_dir).unwrap();
    build_glsl_mod(glsl_dir.join("mod.rs")).unwrap();
    build_ext_instructions(glsl_dir.join("instruction.rs"), &grammar_glsl).unwrap();
    build_enum(glsl_dir.join("enumeration.rs"), &grammar_glsl).unwrap();
}
...
/// Generate basic data for the GLSL instruction set
fn build_glsl_mod<P: AsRef<Path>>(path: P) -> Result<()> {
    let mut dest = ident::IdentWriter::new(File::create(path).unwrap());
...
fn build_instructions<P: AsRef<Path>>(path: P, grammar: &json::JsonValue) -> Result<()> {
    let mut dest = ident::IdentWriter::new(File::create(path).unwrap());
...
/// Generate instruction data for the ext instruction set
fn build_ext_instructions<P: AsRef<Path>>(path: P, grammar: &json::JsonValue) -> Result<()> {
    let mut dest = ident::IdentWriter::new(File::create(path).unwrap());
...
/// Generate enumeration data for the instruction set
fn build_enum<P: AsRef<Path>>(path: P, grammar: &json::JsonValue) -> Result<()> {
    let mut dest = ident::IdentWriter::new(File::create(path).unwrap());

    try!(writeln!(dest, "// WARNING: Generated by `build/main.rs`, do NOT change manually!"));
    try!(writeln!(dest, "use std::collections::HashMap;"));
    try!(writeln!(dest, "use read_binary::{{RawInstructionView, ReadBinaryExt}};"));
    try!(writeln!(dest, "use write_binary::WriteBinaryExt;"));
            ...
            "BitEnum" => {
                ...
                if has_parameters {
                    ...
                    try!(writeln!(dest, "bitflags! {{")); dest.ident(); {
                        try!(writeln!(dest, "pub flags {name}Flags: u32 {{", name=enum_name)); dest.ident(); {
                            for enumerant in op_kind["enumerants"].members() {
                    ...
                    // list of all possible parameter types
                    let mut param_types = HashSet::new();
                    for enumerant in op_kind["enumerants"].members() {
                        for param in enumerant["parameters"].members() {
                            param_types.insert(param["kind"].as_str().unwrap());
                        }
                    }

                    try!(writeln!(dest, "#[derive(Clone, Debug)]"));
                    try!(writeln!(dest, "pub enum {name}Value {{", name=enum_name)); dest.ident(); {
                        for param_ty in &param_types {
                            try!(writeln!(dest, "{ty}({ty}),", ty=param_ty));
                        }
                    } dest.unident(); try!(writeln!(dest, "}}"));
                    try!(writeln!(dest, "impl WriteBinaryExt for {enum_name}Value {{", enum_name=enum_name)); dest.ident(); {
                        try!(writeln!(dest, "fn write(&self, instr: &mut RawInstruction) {{")); dest.ident(); {
                            try!(writeln!(dest, "match *self {{")); dest.ident(); {
                                for param_ty in &param_types {
                    ...
                    try!(writeln!(dest, "#[derive(Clone, Debug)]"));
                    try!(writeln!(dest, "pub struct {name} {{", name=enum_name)); dest.ident(); {
                        try!(writeln!(dest, "pub bits: {name}Flags,", name=enum_name));
                        try!(writeln!(dest, "pub values: HashMap<{name}Flags, Vec<{name}Value>>,", name=enum_name));
                    } dest.unident(); try!(writeln!(dest, "}}"));

                    try!(writeln!(dest, "impl ReadBinaryExt for {enum_name} {{", enum_name=enum_name)); dest.ident(); {
                        try!(writeln!(dest, "fn read(view: &mut RawInstructionView) -> Result<Self, ReadError> {{")); dest.ident(); {
                            try!(writeln!(dest, "let word = try!(u32::read(view));"));
                            try!(writeln!(dest, "if let Some(val) = {enum_name}Flags::from_bits(word) {{", enum_name=enum_name)); dest.ident(); {
                                try!(writeln!(dest, "let mut values = HashMap::new();"));
                                for enumerant in op_kind["enumerants"].members() {
                                    if !enumerant["parameters"].is_null() {
                                        try!(writeln!(dest, "if val.contains({prefix}{name}) {{",
                                            name=enumerant["enumerant"],
                                            prefix=enum_name
                                        )); dest.ident(); {
                                            try!(writeln!(dest, "let mut params = Vec::new();"));
                                            for param in enumerant["parameters"].members() {
                                                try!(writeln!(dest, "params.push({name}Value::{ty}(try!({ty}::read(view))));",
                                                    name=enum_name,
                                                    ty=param["kind"]
                                                ));
                                            }
                                            try!(writeln!(dest, "values.insert({prefix}{name}, params);",
                                                name=enumerant["enumerant"],
                                                prefix=enum_name
                                            ));
                                        } dest.unident(); try!(writeln!(dest, "}}"));
                                ...
                                try!(writeln!(dest, "Ok({enum_name} {{ bits: val, values: values }})", enum_name=enum_name));
                            } dest.unident(); try!(writeln!(dest, "}}"));
                            try!(writeln!(dest, "else {{ Err(ReadError::InvalidEnumValue(word)) }}"));
                        } dest.unident(); try!(writeln!(dest, "}}"));
                    } dest.unident(); try!(writeln!(dest, "}}"));
                            ...
                            for enumerant in op_kind["enumerants"].members() {
                                if !enumerant["parameters"].is_null() {
                                    try!(writeln!(dest, "match self.values.get(&{prefix}{name}) {{",
                                        ...
                                        try!(writeln!(dest, "Some(ref operands) => operands.write(instr),"));
                    ...
                    let ref enum_name = op_kind["kind"];
                    try!(writeln!(dest, "bitflags! {{")); dest.ident(); {
                        try!(writeln!(dest, "pub flags {name}: u32 {{", name=enum_name)); dest.ident(); {
                            for enumerant in op_kind["enumerants"].members() {
                                try!(writeln!(dest, "const {prefix}{name} = {value},",
                                        name=enumerant["enumerant"],
                                        prefix=enum_name,
                    ...
                    try!(writeln!(dest, "impl OperandExt for {enum_name} {{", enum_name=enum_name)); dest.ident(); {
                        try!(writeln!(dest, "fn capabilities(&self) -> Vec<Capability> {{")); dest.ident(); {
                            try!(writeln!(dest, "let mut _capabilities = Vec::new();"));
                            for enumerant in op_kind["enumerants"].members() {
                                if !enumerant["capabilities"].is_null() {
                                    try!(writeln!(dest, "if self.contains({prefix}{name}) {{",
                                        name=enumerant["enumerant"],
                                        prefix=enum_name
                                    )); dest.ident(); {
                                        for capability in enumerant["capabilities"].members() {
